...
public class GlobalDropListener implements Listener {
    ...
    public void onItemSpawn(ItemSpawnEvent event) {
        ...
        if (item.hasItemMeta() && item.getItemMeta().hasCustomModelData()
            ...
            Material blockMat = getBlockMaterial(item.getType());
            // If blockMat is null, it might use fallback logic in calculateStars (Normal
            // Biome)
            ...
            Material targetMat = blockMat;
            // If checking a custom item that isn't a vanilla ore type, mapped blockMat will
            // be null.
            // We should NOT strictly enforce stars on unknown items (like Furniture) just
            // because they have ModelData.
            if (targetMat == null)
                return;
