 ...
 * <p>
 * 管理单个星魂共振台的锻造状态.
 * Refactored to delegate logic to ForgingDifficulty and ForgingScoreCalculator.
 */
public class ForgingSession {
    ...
    // Helpers
    private final ForgingDifficulty difficulty = new ForgingDifficulty();
    private final ForgingScoreCalculator scoreCalculator = new ForgingScoreCalculator();
    ...
    private void initializeVisuals() {
        ...
        Material displayMat = ForgingVisuals.resolveDisplayMaterial(getTargetId());
    ...
    private void updateIdleStatus() {
        if (visuals != null) {
            if (activeRecipe != null) {
                // In progress
                double avgStar = scoreCalculator.calculateAverageStar();

                // Calculate modifier for display
                Player p = StarryForge.getInstance().getServer().getPlayer(playerId);
                double modifier = scoreCalculator.calculateModifier(p);
    ...
    public void processMaterialInput(ItemStack ingot, ForgingRecipeManager.ForgingRecipe recipe) {
        ...
        if (activeRecipe == null) {
                ...
                if (!p.getPersistentDataContainer().has(tutorialKey, PersistentDataType.BYTE)) {
                    p.showTitle(net.kyori.adventure.title.Title.title(
                            MiniMessage.miniMessage()
                                    .deserialize(StarryForge.getInstance().getConfigManager()
                                            .getMessage("forging.session.tutorial_title")),
                            MiniMessage.miniMessage()
                                    .deserialize(StarryForge.getInstance().getConfigManager()
                                            .getMessage("forging.session.tutorial_subtitle")),
                            net.kyori.adventure.title.Title.Times.times(java.time.Duration.ofMillis(500),
                                    java.time.Duration.ofMillis(4000), java.time.Duration.ofMillis(1000))));
                    ...
                    p.showTitle(net.kyori.adventure.title.Title.title(
                            MiniMessage.miniMessage()
                                    .deserialize(StarryForge.getInstance().getConfigManager()
                                            .getMessage("forging.session.start_title")),
                            MiniMessage.miniMessage()
                                    .deserialize(StarryForge.getInstance().getConfigManager()
                                            .getMessage("forging.session.start_subtitle")),
            ...
            if (p != null) {
                p.showTitle(net.kyori.adventure.title.Title.title(
                        ...
                        MiniMessage.miniMessage()
                                .deserialize(StarryForge.getInstance().getConfigManager()
                                        .getMessage("forging.session.phase_title")
                                        .replace("{phase}", String.valueOf(roundsCompleted + 1))),
                        net.kyori.adventure.title.Title.Times.times(java.time.Duration.ofMillis(100),
        ...
        if (ingot.hasItemMeta()) {
            Integer s = ingot.getItemMeta().getPersistentDataContainer().get(NexusKeys.STAR_RATING,
                    PersistentDataType.INTEGER);
            if (s != null) {
                star = s;
            } else {
                // Fallback: Check legacy key just in case
                Integer legacy = ingot.getItemMeta().getPersistentDataContainer().get(
                        new NamespacedKey(StarryForge.getInstance(), "nexus_star_rating"), PersistentDataType.INTEGER);
                if (legacy != null)
                    star = legacy;
            }
        }
        scoreCalculator.addMaterialStar(star);

        Player p = StarryForge.getInstance().getServer().getPlayer(playerId);
        if (p != null) {
            p.sendMessage(MiniMessage.miniMessage().deserialize(StarryForge.getInstance().getConfigManager()
                    .getMessage("forging.session.material_input").replace("{star}", String.valueOf(star))));
        }
        ...
        difficulty.applySettings(roundsCompleted, totalRounds);

        if (visuals != null) {
            visuals.spawnConstellation(difficulty.getTotalNodes(), difficulty.getInterferenceNodes(), 1.2);
            visuals.setRotationSpeed(difficulty.getRotationSpeed());
            visuals.setStatusVisible(false); // Hide text during QTE
        }

        this.hitNodes.clear(); // Reset hit nodes for new round
    ...
    public void tick() {
        ...
        if (isForging()) {
            // Check timeout (Hard Limit: 3x Standard Time to prevent stalling)
            long elapsed = System.currentTimeMillis() - startTime;
            long timeLimit = difficulty.getTimeLimit();

            if (elapsed > timeLimit * 3) {
                // Hard Time's up! Finish round with what we have.
                finishRound(null);
            } else {
                // ActionBar Update
                if (playerId != null) {
                    ...
                    if (p != null && p.isOnline()) {
                        double progress = (double) hitNodes.size() / difficulty.getTotalNodes();
                        double timeRem = (timeLimit - elapsed) / 1000.0;

                        String timeColor = timeRem >= 0 ? "<green>" : "<red>";
                        String timeStr = timeColor + String.format("%.1f", Math.abs(timeRem)) + "s";
                        if (timeRem < 0)
                            timeStr = "<red>OVERTIME " + timeStr;
                        ...
                        p.sendActionBar(MiniMessage.miniMessage().deserialize(
                                StarryForge.getInstance().getConfigManager()
                                        .getMessage("forging.session.actionbar_timer")
    ...
    public void handleHit(Player player) {
        ...
        if (!player.getUniqueId().equals(playerId)) {
            player.sendMessage(MiniMessage.miniMessage()
                    .deserialize(StarryForge.getInstance().getConfigManager().getMessage("forging.session.not_owner")));
            return;
        }
        ...
        if (hammerTier <= 0) {
            player.sendMessage(MiniMessage.miniMessage().deserialize(
                    StarryForge.getInstance().getConfigManager().getMessage("forging.session.need_hammer")));
            return;
        }
        ...
        List<Location> nodes = visuals.getNodeLocations();
        int bestIndex = -1;
        ...
        // 2. Check Interference Hits (Void Rifts)
        List<Location> badNodes = visuals.getInterferenceLocations();
        int badIndex = -1;
        double badDot = bestDot; // Same threshold

        for (int i = 0; i < badNodes.size(); i++) {
            Location badLoc = badNodes.get(i);
            if (badLoc.distanceSquared(eyeLoc) > maxDist * maxDist)
                continue;

            org.bukkit.util.Vector toNode = badLoc.toVector().subtract(eyeLoc.toVector()).normalize();
        ...
        if (badIndex != -1) {
            // Hit a Void Rift!
            visuals.setInterferenceHit(badIndex);

            // Penalty
            currentRoundScore -= 0.2; // -20% score
            if (currentRoundScore < 0)
                currentRoundScore = 0;

            // Visual feedback
            player.sendMessage(MiniMessage.miniMessage().deserialize(
                    "<red><bold>警告!</bold> <gray>触碰虚空裂隙! 品质严重受损!"));
            player.playSound(player.getLocation(), org.bukkit.Sound.ENTITY_WARDEN_DIG, 1.0f, 0.5f);

            // Apply Blindness for immersion
            player.addPotionEffect(new org.bukkit.potion.PotionEffect(org.bukkit.potion.PotionEffectType.BLINDNESS, 20,
                    0, false, false, false));

            return; // Stop processing, don't check good nodes if we hit bad one
        }

        // 3. Check Target Hits
        for (int i = 0; i < nodes.size(); i++) {
            if (hitNodes.contains(i))
                continue; // Skip already hit nodes

            Location nodeLoc = nodes.get(i);
            // Check distance first to avoid hitting far nodes through walls
            if (nodeLoc.distanceSquared(eyeLoc) > maxDist * maxDist)
                continue;
        ...
        if (bestIndex != -1) {
            ...
            double nodeScore = 1.0 / difficulty.getTotalNodes();
            currentRoundScore += nodeScore;

            // Check Win Condition
            if (hitNodes.size() >= difficulty.getTotalNodes()) {
                // All hit! Check time.
                long elapsed = System.currentTimeMillis() - startTime;
                long timeLimit = difficulty.getTimeLimit();

                // Normalize to exactly 1.0
                currentRoundScore = 1.0;

                if (elapsed <= timeLimit) {
                    // Perfect (Within Time)
                    player.sendMessage(MiniMessage.miniMessage().deserialize(
                            StarryForge.getInstance().getConfigManager().getMessage("forging.session.perfect_hit")));
                } else {
                    // Overtime Penalty
                    double overtimeSec = (elapsed - timeLimit) / 1000.0;
                    // Deduct 5% per second of overtime
                    double penalty = overtimeSec * 0.05;
                    currentRoundScore = Math.max(0.0, 1.0 - penalty);

                    player.sendMessage(MiniMessage.miniMessage().deserialize(
                            "<red>超时完成! <gray>品质修正: <red>-" + String.format("%.1f%%", penalty * 100)));
                }
                finishRound(player);
            }
        }
    ...
    private void finishRound(Player player) {
        // 1. Record Score
        scoreCalculator.addQteScore(currentRoundScore);
        roundsCompleted++;
        ...
        if (visuals != null) {
            ...
            visuals = new ForgingVisuals(anvilLocation,
                    new ItemStack(ForgingVisuals.resolveDisplayMaterial(getTargetId())));
            // Re-init base visuals (hologram)
            initializeVisuals();
    ...
    private void finishSession(Player player) {
        ...
        double avgMaterialStar = scoreCalculator.calculateAverageStar();
        double modifier = scoreCalculator.calculateModifier(player);
        int finalStar = scoreCalculator.calculateFinalStar(modifier);
        ...
        if (player != null) {
            ...
            player.sendMessage(MiniMessage.miniMessage().deserialize(
                    "<gray>材料均质: <yellow>" + String.format("%.1f", avgMaterialStar) +
                            " <gray>| QTE修正: <" + (modifier >= 0 ? "green" : "red") + ">"
    ...
    public void restoreLists(List<Integer> stars, List<Double> scores) {
        scoreCalculator.restoreLists(stars, scores);
    }
    ...
    public ItemStack getTargetBlueprint() {
        return targetBlueprint;
    }

    public ItemStack getCurrentIngot() {
        return currentIngot;
    }
    ...
    public List<Integer> getMaterialStars() {
        return scoreCalculator.getMaterialStars();
    }

    public List<Double> getQteScores() {
        return scoreCalculator.getQteScores();
    }
}
