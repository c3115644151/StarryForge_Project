...
*   **交互逻辑**:
    ...
    *   **锻打 (QTE - 星座共鸣)**:
        *   手持 **锻造锤 (Forging Hammer)**。
        *   **机制**: **星位校准 (Constellation Alignment)**。
            *   每一轮锻造开始时，祭坛上方会生成由粒子构成的 **星图 (Constellation)** (基于斐波那契球体算法分布)。
            *   **操作**: 玩家需要抬头观察空中的星点，将准星对准 **青色脉冲星点** 并 **左键挥舞** 锤子（无需点击实体，只需朝向正确）。
            *   **判定**: 系统计算玩家视线向量与星点位置的点积 (Dot Product)，判定是否命中。
            *   **限时**: 每轮锻造有时间限制 (默认 15-20秒)，超时未击破所有星点将强制结算当前轮次。
        *   **锤子等级**: 不同神器需要不同等级 (T1/T2/T3) 的锤子才能锻造。
        *   **动态难度 (Phase Progression)**:
            *   随着锻造进度的推进，QTE 难度会阶梯式上升。
            *   **Phase 1 (0-33%)**: 静止星空，5个节点，时间充裕。
            *   **Phase 2 (33-66%)**: 星体公转 (Orbit)，7个节点，节点缓慢旋转，需预判射击。
            *   **Phase 3 (66-100%)**: 坍缩临界，9个节点，高速旋转，时间紧迫。
            *   **干扰项 (Void Rifts)**: 在高难度阶段，星图附近会出现 **紫色虚空裂隙**。
                *   若误触裂隙（准星扫过），将触发 **虚空崩塌**。
                *   **惩罚**: 扣除 **20%** 当前轮次评分，并给予玩家短暂的 **失明 (Blindness)** 效果。

#### 3.9.2 视觉特效系统 (`ForgingVisuals`)
*   **全息投影**: 使用 `ItemDisplay` 在祭坛上方悬浮显示当前锻造的目标成品，随锻造进度旋转。
*   **星图系统**:
    *   **生成**: 在祭坛上方高空 (Offset Y) 生成 3D 粒子球体。
    *   **待激活星点**: 青色 (Aqua) 脉冲粒子，随时间轻微浮动。
    *   **已激活星点**: 命中后变为 金色 (Gold) 并播放 `HAPPY_VILLAGER` 粒子，同时射出一道光束连接至中央全息投影。
*   **状态反馈**:
    *   **TextDisplay**: 在全息投影上方显示当前进度、品质评分及 QTE 修正值。
    *   **ActionBar**: 实时显示 QTE 剩余时间与星点击破进度条。
*   **完成特效**: 锻造完成后，星图消散，播放史诗级音效 (`UI_TOAST_CHALLENGE_COMPLETE`)，生成环绕式 `WAX_ON` 粒子光环。
...
#### 3.9.4 品质判定公式 (Quality Calculation)
*   **基础品质 (Base Quality)**: 所有投入材料的星级平均值 (`AvgMaterialStar`)。
*   **QTE 评分 (QTE Score)**:
    *   每轮 QTE 满分为 1.0 (在规定时间内击破所有星点)。
    *   **超时惩罚**: 若超出规定时间，每秒扣除 5% 的分数。
    *   最终得分为所有轮次的平均分 (`AvgQteScore`)。
*   **修正系数 (Modifier)**:
    *   公式: `Modifier = AvgQteScore - 0.5`
    *   **上限**: +10% (0.10) (可通过玩家锻造等级提升)。
    *   **下限**: -50% (-0.50)。
    *   这意味着 QTE 评分需达到 **0.6** 以上才能获得正向加成，低于 0.5 会导致品质下降。
*   **最终星级**:
    *   `FinalStar = AvgMaterialStar * (1.0 + Modifier)`
    *   结果四舍五入。
